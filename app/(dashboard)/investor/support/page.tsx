...
export default function InvestorSupportPage() {
  ...
  function loadTickets() {
    api<Ticket[]>("/investor/support/tickets").then(setTickets).catch(() => setTickets([]));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api("/investor/support/tickets", { method: "POST", body: { category, subject, message } });
      setSent(true);
      setSubject("");
      setMessage("");
  ...
  return (
    <>
      <PageHeader
        title="Get Support"
        description="Reach the Rydora team for help with your account or investments."
        breadcrumb={[{ label: "Dashboard", href: "/investor" }, { label: "Support" }]}
      />
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={5}
                placeholder="Describe your issue in detail — include vehicle plate, dates, or amounts where relevant"
